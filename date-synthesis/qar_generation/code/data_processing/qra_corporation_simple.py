...
def _extract_references_from_qa(qa: Dict) -> str:
    """从单条 QA 中提取 ref / refs 作为 references 字段。

    - 如果存在列表类型的 "ref" 或 "refs"，按行用 "\n" 拼接；
    - 如果是单个字符串则直接转成字符串；
    - 不再回退到整篇 doc，只使用标注好的 ref 片段。
    """
...
def main():
    ...
    output_file_json = os.path.join(args.output_dir, "QA-summary-test.json")
