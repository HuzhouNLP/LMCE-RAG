...
class LKIFPhi3Attention(nn.Module):
    ...
    def forward(
        ...
        # Here we add the LKIF key values to the key and value states
        if lkif_kvs is not None:
            if (
                self.layer_idx % lkif_layer_frequency == 0
                ...
                attention_mask = torch.concat([lkif_atten_mask, attention_mask], dim=-1)
...
class LKIFPhi3ForCausalLM(Phi3PreTrainedModel):
