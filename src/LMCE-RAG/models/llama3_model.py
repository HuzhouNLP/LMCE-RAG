...
class LkifLlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            # 兼容不同配置格式：优先使用 "type"，如果不存在则使用 "rope_type"
            ...
            if scaling_type == "linear":
                ...
                # 对于Llama3类型的RoPE，使用标准的RotaryEmbedding
                self.rotary_emb = LlamaRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
    ...
    def forward(
        ...
        **kwargs,
        ...
        lkif_layer_frequency = lkif_config.kb_layer_frequency
        dynamic_sparsify = lkif_config.dynamic_sparsify
        topk_size = lkif_config.top_k_kb
        attn_weights_2 = None
        if lkif_kvs is not None:
            if self.layer_idx % lkif_layer_frequency == 0:
                lkif_keys, lkif_values = (
                    lkif_kvs  # (lkif_len, head_dim * num_heads * num_adapters)
                )
                lkif_idx = (
                    self.layer_idx // lkif_layer_frequency
                )  # Should be something inside the lkif config
                if len(lkif_keys.shape) == 2:  # Not batch dim
                    lkif_len = lkif_keys.shape[0]
                    lkif_keys = lkif_keys.reshape(
                        lkif_len,
                        1 + self.config.num_hidden_layers // lkif_layer_frequency,
                        -1,
                    )[:, lkif_idx]
                    lkif_values = lkif_values.reshape(
                        lkif_len,
                        1 + self.config.num_hidden_layers // lkif_layer_frequency,
                        -1,
                    )[:, lkif_idx]
                    lkif_keys = lkif_keys.view(
                        lkif_len, self.num_heads, self.head_dim
                    ).transpose(0, 1)
                    lkif_values = lkif_values.view(
                        lkif_len, self.num_heads, self.head_dim
                    ).transpose(0, 1)
                    lkif_keys = lkif_keys.unsqueeze(0).expand(
                        bsz, self.num_heads, lkif_len, self.head_dim
                    )
                    lkif_values = lkif_values.unsqueeze(0).expand(
                        bsz, self.num_heads, lkif_len, self.head_dim
                    )
                    if dynamic_sparsify:
                        lkif_keys, lkif_values, attn_weights_2 = self.prune_key_value(
                            query_states_2, lkif_keys, lkif_values, topk_size
                        )
                    # Append the LKIF keys and values in the front, in front of padding
                    key_states = torch.concat([lkif_keys, key_states], dim=2)
                    value_states = torch.concat([lkif_values, value_states], dim=2)
                elif len(lkif_keys.shape) == 3:  # Has a batch dim
                    ...
                    lkif_keys = lkif_keys.view(
                        bsz, lkif_len, self.num_heads, self.head_dim
                    ).transpose(1, 2)
                    lkif_values = lkif_values.view(
                        bsz, lkif_len, self.num_heads, self.head_dim
                    ).transpose(1, 2)
                    ...
                    key_states = torch.concat([lkif_keys, key_states], dim=2)
                    value_states = torch.concat([lkif_values, value_states], dim=2)
                # Modify the attention matrix: Append a (seq_len, lkif_len) block to the left
                lkif_len = lkif_keys.shape[2]
                # 在部分实现（如 SDPA + 无 padding）中，causal_mask 可能为 None，此时直接使用 is_causal 逻辑，
                # 不需要也无法基于 attention_mask 构造额外的 LKIF mask。
                if attention_mask is not None:
                    ...
                    lkif_atten_mask = (
                        padding_mask * PADDING_VALUE + (~padding_mask) * lkif_atten_mask
                    )
                    attention_mask = torch.concat(
                        [lkif_atten_mask, attention_mask], dim=-1
                    )

        attn_weights = torch.matmul(
            query_states, key_states.transpose(2, 3)
        ) / math.sqrt(self.head_dim)
        sep_query_head = lkif_config.sep_query_head
        lkif_scale_factor = lkif_config.kb_scale_factor
        if sep_query_head:
            if lkif_kvs is not None:
                if self.layer_idx % lkif_layer_frequency == 0:
                    # If we have pruned the LKIF tokens, then this quantity should have been computed,
                    # if not, then we compute it here
                    if attn_weights_2 is None:
                        attn_weights_2 = torch.matmul(
                            query_states_2, lkif_keys.transpose(2, 3)
                        ) / math.sqrt(self.head_dim)
                    attn_weights = attn_weights[:, :, :, lkif_len:]
                    if lkif_scale_factor is not None:
                        attn_weights_2 = (
                            attn_weights_2 - np.log(lkif_len) + np.log(lkif_scale_factor)
                        )
...
LLAMA_ATTENTION_CLASSES = {
    "eager": LkifLlamaAttention,
    "flash_attention_2": LkifLlamaAttention,
    "sdpa": LkifLlamaAttention,
}
...
class LkifLlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if config._attn_implementation == "flash_attention_2":
            raise NotImplementedError(
                "Flash Attention 2 is not yet supported for LKIF."
            )
    ...
    def get_lkif_config(self):
        return self.config

    def set_lkif_config(self, config):
        self.config = config
