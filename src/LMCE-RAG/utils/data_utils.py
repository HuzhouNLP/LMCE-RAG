...
def augment_row(row: dict, dataset_type: str = "game") -> str:
    """Generate a question using templates, supporting both game and law datasets."""
    templates = [
        "What is the {} of {}?",
    ...
    if dataset_type == "law":
        # For law dataset, use the original query content or generate variations
        original_query = row.get("query", {}).get("content", "")
        if original_query:
            return original_query
        else:
            # Fallback to template-based generation if needed
            domain = row.get("domain", "legal matter")
            query_type = row.get("query", {}).get("query_type", "information")
            tid = np.random.randint(0, len(templates))
            return templates[tid].format(query_type.lower(), domain.lower())
    else:
        # Original game dataset logic
