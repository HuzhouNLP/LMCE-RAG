...
class LKIFEncoder(nn.Module, FeatureExtractionMixin):
    # Special tokens used by LKIF encoding
    lkif_special_token = {
        "<LKIF_BEGIN>": 0,
        "<LKIF_END>": 1,
        "<KEY_SEP>": 2,
    ...
    def __init__(
        ...
        if encoder_name in ["OAI", "BigOAI"]:
            ...
            # If a local model name is provided, use a safe relative/example path
            if encoder_name == "all-MiniLM-L6-v2":
                model_path = "/home/user/lkif/all-MiniLM-L6-v2"
            else:
                model_path = encoder_name

            self.base_model = SentenceTransformer(model_path)
        ...
        # Use class-level LKIF special token mapping
        self.embedding = nn.Embedding(len(self.lkif_special_token), out_dim)
        self.device = device
        self.to(self.device)
    ...
    def get_special_token_embd(self, token_type):
        """
        Get the embedding for the special token. Accepts a string and returns a tensor.
        """
        idx = torch.tensor(self.lkif_special_token[token_type]).to(
